...
namespace Libery_Frontend.Views
{
    ...
    public partial class E_Media : ContentPage
    {
        ...
        private async void AllProdButton_Clicked(object sender, EventArgs e)
        {
            ...
            await Task.WhenAll(
                FirstListView.FadeTo(1, 1000),
                SecondListView.FadeTo(0, 500),
                ThirdListView.FadeTo(0, 500)
            );
        ...
        private async void EBooksButton_Clicked(object sender, EventArgs e)
        {
            ...
            MainThread.BeginInvokeOnMainThread(
                async () =>
                {
                    SecondListView.ItemsSource = await GetBooksAsync();
                }
            );

            await Task.WhenAll(
                SecondListView.FadeTo(1, 1000),
                FirstListView.FadeTo(0, 500),
                ThirdListView.FadeTo(0, 500)
            );
        ...
        private async void EMoviesButton_Clicked(object sender, EventArgs e)
        {
            ...
            MainThread.BeginInvokeOnMainThread(
                async () =>
                {
                    ThirdListView.ItemsSource = await GetMovieAsync();
                }
            );

            await Task.WhenAll(
                ThirdListView.FadeTo(1, 1000),
                SecondListView.FadeTo(0, 500),
                FirstListView.FadeTo(0, 500)
            );
